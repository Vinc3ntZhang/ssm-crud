...
<head>
    ...
    <%--  web路径：
      以/开始的相对路径，找资源，以当前支援的路径为基准，进常容易出问题。
      开始的相对路径，找资源，以服务器的路径为标准（http://localhost:3306）;需要加上项目名
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <label class="radio-inline">
                                <input type="radio" name="gander" id="gander1_add_input" value="M" checked="checked"> 男
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="gander" id="gander2_add_input" value="W"> 女
                            </label>
                        </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">添加关闭</button>
            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <label class="radio-inline">
                                <input type="radio" name="gander" id="gander1_update_input" value="M" checked="checked">
                                男
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="gander" id="gander2_update_input" value="W"> 女
                            </label>
                        </div>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    ...
    function build_emps_table(result) {
        ...
        $.each(emps, function (index, item) {
            ...
            var ganderTd = $("<td></td>").append(item.gander == 'M' ? "男" : "女");
            var emailTd = $("<td></td>").append(item.email);
            var deptNameTd = $("<td></td>").append(item.department.deptName);
            var editBtn = $("<button></button>").addClass("btn btn-primary btn_sm edit_btn").append("<span></span>").addClass("glyphicon glyphicon-plus").append("编辑");
            //为编辑按钮添加一个自定的属性，来表示当前员工ID
            editBtn.attr("edit-id", item.empId);
            var deleteBtn = $("<button></button>").addClass("btn btn-danger btn_sm delete_btn").append("<span></span>").addClass("glyphicon glyphicon-trash").append("删除");
    ...
    ​​
                            depts: (2) […]
    ​​​
                                0: Object { deptId: 1, deptName: "开发部" }
    ​​​
                                    1: Object { deptId: 2, deptName: "测试部" }
    ​​​
                            length: 2
                */
                //显示部门信息在下拉列表中
    ...
    //(1)、可以创建按钮的时候绑定。（2）、绑定单击事件.live();
    //jquery新版没有liva，使用on替代
    ...
    function getEmp(id) {
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#empUpdateModal input[name=gander]").val([empDate.gander]);
    ...
    $("#emp_update_btn").click(function () {
        ...
        var regEmail = /^\w+@[0-9a-zA-Z]{2,4}\.[a-zA-Z]{2,3}(\.[a-zA-Z]{2,3})?$/;
        console.log(email);
        if (!regEmail.test(email)) {
            // alert("邮箱格式不正确");
            // $("#email_add_input").parent().addClass("has-error");
